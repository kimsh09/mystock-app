...
if pure_code:
    ...
    with sim_col2:
        ...
        # 1. 🚨 변수 선언 순서 보정: 계산에 필요한 주가 변수를 먼저 정의합니다.
        # (만약 기존 코드에서 현재 주가를 담는 변수명이 다르면 이 부분을 파트너님 기존 변수명에 맞게 매칭해야 합니다.)
        latest_price_tmp = float(target_display_price) if 'target_display_price' in locals() else 0.0
        if latest_price_tmp == 0.0 and 'df' in locals() and not df.empty:
        latest_price_tmp = float(df['Close'].iloc[-1])

        current_avg_price = tmp_cost_sum / tmp_qty_sum if tmp_qty_sum > 0 else 0

        # 2. 정밀 분할매수 역산 및 예외 처리 조건문
